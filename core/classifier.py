...
UPDATED:
- LYING is now vote-based (not strict all-gates)
- thresholds come from PostureConfig (loosened defaults)
"""
...
class PostureClassifier:
    ...
    def classify(self, kpts: Dict, frame_height: Optional[int] = None) -> Tuple[str, float]:
        ...
        segments_aligned = (
            leg_angle is not None
            and self.geo.are_segments_aligned(
        ...
        for side in ["left", "right"]:
            ...
            if hip_point and knee_point and ankle_point:
                knee_angles.append(
                    self.geo.calculate_angle_between_three_points(
        ...
        if self.frame_height and len(xs) >= self.config.lying_min_points:
            width = max(xs) - min(xs)
            ...
            if torso_dev_horiz <= self.config.lying_torso_dev_horiz_max:
                votes += 1
            if body_axis_dev_horiz is not None and body_axis_dev_horiz <= self.config.lying_body_axis_dev_horiz_max:
                votes += 1
            ...
            # softness
            if body_compactness is not None and body_compactness < 0.60:
                votes += 1
            if is_inverted:
                votes += 1
        ...
        if avg_knee_angle is not None and avg_knee_angle < self.config.sitting_knee_angle_max:
            sitting_count += 1
        if (
            torso_dev_vert < self.config.vertical_deviation_threshold * 1.5
            and leg_dev_vert is not None
            and leg_dev_vert > self.config.bending_threshold
            and not segments_aligned
        ):
            sitting_count += 1
        ...
        # Standing
        if (
            torso_dev_vert < self.config.vertical_deviation_threshold
            and leg_dev_vert is not None
            and leg_dev_vert < self.config.vertical_deviation_threshold
            and segments_aligned
            and not is_inverted
        ):
            confidence = 0.85
        ...
        if (
            torso_dev_vert < self.config.vertical_deviation_threshold * 1.3
            and max_knee_angle is not None
            and max_knee_angle > self.config.standing_knee_angle_min
            and hip_height_ratio is not None
            and hip_height_ratio > 0.38
        ):
            return (PostureLabel.STANDING, 0.8)

        # Bending
        if (
            leg_dev_vert is not None
            and leg_dev_vert < self.config.vertical_deviation_threshold * 1.5
            and segments_diff > self.config.bending_threshold
        ):
